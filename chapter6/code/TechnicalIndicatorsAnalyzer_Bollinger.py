...
class TechnicalIndicatorsAnalyzer:
      ...
      def getBollingerBands(self,stock_data, window):
          stock_MA = stock_data.Close.rolling(window=window).mean()
          stock_SD = stock_data.Close.rolling(window=window).std()
          stock_data['MiddleBand'] = stock_MA
          stock_data['UpperBand'] = stock_MA + (2 * stock_SD) 
          stock_data['LowerBand'] = stock_MA - (2 * stock_SD)
          return stock_data
      ...
      def createBollingerBandsPlot(self,stock_data,bollinger_bands):
          plot.figure(figsize=(10,7))
          plot.title('Bollinger Bands')
          plot.xlabel('Date')
          plot.ylabel('Price')

          plot.plot(bollinger_bands.Close,lw=1, label='Close Price')
          plot.plot(stock_data['UpperBand'],'g',lw=1, label='Upper band')
          plot.plot(stock_data['MiddleBand'],'r',lw=1, label='Middle band')
          plot.plot(stock_data['LowerBand'],'g', lw=1, label='Lower band')
...
def main():
    ...
    n = 50
    bollinger_bands = priceIndicator.getBollingerBands(data, n)
    
    priceIndicator.createBollingerBandsPlot(data,bollinger_bands)
