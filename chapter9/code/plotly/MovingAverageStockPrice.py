...
class MovingAverageStockPrice:
      ...
      def createPlot(self,prices, stockSymbol):
          ...
          #fig = plotx.line(x=prices.loc[prices['prices']==1].index,y=prices['Close'][prices['prices']==1],labels='LONG')
          #,lw=0,marker='^',c='g'
          fig = plotx.line(x=prices.index,y=prices["Close"],labels={'x':'Date','y':'StockPrice'})
